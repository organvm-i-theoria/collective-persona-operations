...
The organvm system enforces a strict dependency flow: ORGAN-I (Theory) feeds into ORGAN-II (Art), which feeds into ORGAN-III (Commerce). ORGAN-IV (Orchestration) governs all organs. No back-edges are permitted (e.g., ORGAN-III cannot depend on ORGAN-II). `collective-persona-operations` must define its integration points within this constraint.
